// ==UserScript==
// @name         Steven's MPP script
// @namespace    http://www.multiplayerpiano.com/
// @notice       MPP script by Steven
// @Version      4.3.2
// @description  Modded script for MPP
...
gClient.emit("a", {m: "a", a: message, t: Date.now(), p: {name: "Local", color: "#000"}}); // Emit Message To Client Only
...
function scriptError(msg) {
    console.log("%c[SCRIPT ERROR]: " + msg, "background: #000000; color: #ff0000");
}

function scriptMessage(msg) {
    console.log("%c[SCRIPT MESSAGE]: " + msg, "background: #000000; color: #00ff00");
}
/* [END] Unknown */

/* [START] Get Start Up Delay */
var startUpDelay = window.performance.now();
/* [END] Get Start Up Delay */
...
var scriptVersion = GM_info.script.version;                                                                       // Script version
var gSeeOwnCursor = false;                                                                                        // See own cursor
var test_mode = false;                                                                                            // Test mode
var gMidiOutTest = false;                                                                                         // TODO: This is no longer needed
var gHideKeyColor = false;                                                                                        // Hides key colors when played
var defaultBackground = "http://cdn.pcwallart.com/images/dark-abstract-1920x1080-wallpaper-2.jpg";                // Default background
...
////////////////////////////////////////////////////////////////
	...
	AudioEngineWeb.prototype.actualPlay = function(id, vol, time, part_id) { //the old play(), but with time insted of delay_ms.
		...
		if(this.playings[id]) {
			...
			playing.gain.gain.linearRampToValueAtTime(0.000, time + 0.002);
			playing.source.stop(time + 0.0021);
...
////////////////////////////////////////////////////////////////
	...
	CanvasRenderer.prototype.resize = function(width, height) {
		...
		this.keyMovement = Math.floor(this.whiteKeyHeight * 0.050);
...
////////////////////////////////////////////////////////////////
	...
	(function() {
		...
		gClient.on("count", function(count) {
			if(count > 0) {
				...
				document.title = gClient.desiredChannelId + " (" + count + ")";
			} else {
	...
	(function() {
		gClient.on("participant added", function(part) {
            scriptMessage(part.name + " connected to " + gClient.channel._id);
		...
		gClient.on("participant removed", function(part) {
            scriptMessage(part.name + " disconnected from " + gClient.channel._id);
	...
	gClient.on("n", function(msg) {
        scriptMessage("Recieved note from " + gClient.findParticipantById(msg.p).name + " at " + gClient.channel._id);
	...
	setInterval(function() {
		for(var id in gClient.ppl) {
			...
			if(part.cursorDiv && (Math.abs(part.x - part.displayX) > 0.1 || Math.abs(part.y - part.displayY) > 0.1)) {
				part.displayX += (part.x - part.displayX) * 0.1;
				part.displayY += (part.y - part.displayY) * 0.1;
	...
	(function() {
		function setColor(hex) {
			...
			iv = setInterval(function() {
				document.body.style.background = " url('" + defaultBackground+ "') no-repeat center 90%";
                bottom.style.background = color1.toHexa();
				if(++step >= steps) {
					clearInterval(iv);
					document.body.style.background = " url('" + defaultBackground+ "') no-repeat center 90%";
					bottom.style.background = color1.toHexa();
	...
	(function() {
		...
		var participantMenu = function(part) {
			...
			// add menu items
            if(part.id == gClient.participantId) {
                $('<div class="menu-item re-name">Rename</div>').appendTo(menu)
            ...
            $('<div class="menu-item get-id">ID</div>').appendTo(menu)
            .on("mousedown touchstart", function(evt) {
                copyToClipboard(part._id);
                new Notification({id: "info", title: "Info", text: "Copied ID to clipboard.", target: "#bottom", duration: 1000});
            });
			if(gPianoMutes.indexOf(part._id) == -1) {
				...
				.on("mousedown touchstart", function(evt) {
					...
					$(part.nameDiv).addClass("muted-notes");
                    new Notification({id: "info", title: "Info", text: "Muted " + part.name + " notes.", target: "#piano", duration: 1000});
				});
			} else {
				...
				.on("mousedown touchstart", function(evt) {
					...
					$(part.nameDiv).removeClass("muted-notes");
                    new Notification({id: "info", title: "Info", text: "Unmuted " + part.name + " notes.", target: "#piano", duration: 1000});
				});
			}
			if(gChatMutes.indexOf(part._id) == -1) {
				...
				.on("mousedown touchstart", function(evt) {
					...
					$(part.nameDiv).addClass("muted-chat");
                    new Notification({id: "info", title: "Info", text: "Muted " + part.name + " chat.", target: "#bottom", duration: 1000});
				});
			} else {
				...
				.on("mousedown touchstart", function(evt) {
					...
					$(part.nameDiv).removeClass("muted-chat");
                    new Notification({id: "info", title: "Info", text: "Unmuted " + part.name + " chat.", target: "#bottom", duration: 1000});
				});
			}
			if(!(gPianoMutes.indexOf(part._id) >= 0) || !(gChatMutes.indexOf(part._id) >= 0)) {
				...
				.on("mousedown touchstart", function(evt) {
					...
					$(part.nameDiv).addClass("muted-notes");
					$(part.nameDiv).addClass("muted-chat");
                    new Notification({id: "info", title: "Info", text: "Muted " + part.name + " completely.", target: "#bottom", duration: 1000});
				});
			}
			if((gPianoMutes.indexOf(part._id) >= 0) || (gChatMutes.indexOf(part._id) >= 0)) {
				...
				.on("mousedown touchstart", function(evt) {
					...
					$(part.nameDiv).removeClass("muted-notes");
					$(part.nameDiv).removeClass("muted-chat");
                    new Notification({id: "info", title: "Info", text: "Unmuted " + part.name + " completely.", target: "#bottom", duration: 1000});
				});
			}
			if(gClient.isOwner()) {
				...
				.on("mousedown touchstart", function(evt) {
                    ...
                    new Notification({id: "info", title: "Info", text: "Gave the crown to " + part.name + ".", target: "#bottom", duration: 1000});
				});
				...
				.on("mousedown touchstart", function(evt) {
                    ...
                    new Notification({id: "info", title: "Info", text: "Kickbanned " + part.name + ".", target: "#bottom", duration: 1000});
				});
			}
			menu.fadeIn(100);
		};
	...
	var chat = (function() {
		...
		return {
			...
			receive: function(msg) {
				...
				var li = $('<li><span class="time"/><span class="name"/><span class="message"/>');
                
                li.find(".time").text("[" + new Date(msg.t).toTimeString().split(' ')[0] + "]");
...
////////////////////////////////////////////////////////////////
    ...
    $("body #bottom .relative").append('<div id="copy-btn" class="ugly-button copy_btn" style="position: fixed;bottom: 35px;width: 10px" title="Copy Room Name">C</div>');
    $("#copy-btn").click(function() {
        copyToClipboard(gClient.desiredChannelId);
        new Notification({id: "info", title: "Info", text: "Copied room name to clipboard.", target: "#bottom", duration: 1000});
    });
    $("body #bottom .relative").append('<div id="console-btn" class="ugly-button console_btn" style="position: fixed;bottom: 35px;width: 18px;height: 14px;right: 800px;background: url(https://puu.sh/s8mZU/f4c576da92.png)" title="Console"></div>');
    $("#console-btn").click(function() {
        var html = document.createElement("div");
        html.innerHTML = '<textarea id="code" rows="5" cols="54" style="resize: none;"></textarea><br><button id="execute" type="button">Execute</button>';
        notification = new Notification({title: "Console | By BN-TK | Version 1.0", html: html, duration: -1, target: "#console-btn"});
        notification.on("close", function() {
            notification = null;
        });
        $("#execute").click(function() {
            notification = null;
            scriptMessage(document.getElementById("code").value);
            var eval_code = new Function(document.getElementById("code").value)();
            gClient.emit("a", {m: "a", a: eval_code, t: Date.now(), p: {name: "Console", color: "#00ff00"}});
        });
    });
    $("body #bottom .relative").append('<div id="background-btn" class="ugly-button background_btn" style="position: fixed;bottom: 35px;right: 200px">Background</div>');
    $("#background-btn").click(function() {
        var html = document.createElement("div");
        html.innerHTML = '<textarea id="bg-url" rows="5" cols="54" style="resize: none;"></textarea><br><button id="set-bg" type="button">Set</button>';
        notification = new Notification({title: "Background | By BN-TK | Version 1.0", html: html, duration: -1, target: "#background-btn"});
        notification.on("close", function() {
            notification = null;
        });
        $("#set-bg").click(function() {
            notification = null;
            defaultBackground = document.getElementById("bg-url").value;
            document.body.style.background = " url('" + defaultBackground+ "') no-repeat center 90%";
        });
    });
});
/* [END] Modded Script.js */

/* [START] Unknown */
var startUpTime = window.performance.now() - startUpDelay;
scriptMessage("Script startup time: " + startUpTime);
/* [END] Unknown */
        textarea.select();
        try {
            return document.execCommand("copy");
